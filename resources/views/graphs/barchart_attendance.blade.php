...
@section('content')
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            @php
              foreach($attendance_model as $model) {
                  echo "['".$model->fullname."', ".$model->attendance."],";
              }
            @endphp
        ]);
 
        var options = {
            chart: {title: 'Bar Graph | Attendance',subtitle: 'Id, Attendance',}, 
            colors: ['hotpink','red'],
            bars: 'vertical'
        };
  ...
  <body>
    <div class="ml-12 mr-12 mt-9">
        <h2 class="text-2xl font-extrabold flex justify-center mb-6">Class {{  $class->name  }} Attendance Rates</h2>
    </div>
    <form action="{{  route('graph')  }}" method="post">
        ...
        <input name="class_id" value="{{$class->id}}" type="hidden">
            ...
            <select name="graphtype" id="graphtype" class="p-3 text-black text-bold bg-gray-200 border-2 rounded-lg">
                ...
                <option value="Student Attendance" selected>Student Attendance</option>
