...
@section('content')
    <div class="flex justify-center mt-48 mb-48 text-extrabold">
        <div class="w-4/12 bg-gray-700 text-white p-6 rounded-lg">
            <h2 class="text-2xl my-4 flex justify-center"><b>Edit Role</b></h2>
            <form action="{{ route('modify_role') }}" method="post">
                @csrf
                <input name="original" value="{{$role->name}}" type="hidden">
                <input name="role_id" value="{{ $role->id }}" type="hidden">
                <div class="mb-4">
                    <label for="rolename">Role name: </label>
                    <input type="text" name="name" id="name" placeholder="Enter Role Name" class="p-3 w-full text-black text-bold bg-gray-200 border-2 @error('name') border-red-500 @enderror rounded-lg" value="{{ $role->name }}">
                    @error('name')
