...
@section('content')
  <div class="ml-12 mr-12">
    <p class="text-2xl font-extrabold flex justify-center mb-6">Add Students</p>
    <div class="px-16 py-2 flex justify-center">
      <form action="{{ route('class.students.add', $class->id)  }}" method='get'>
      ...
      <form action="{{ route('class.students', $class->id)  }}" method="any">
        <span class="flex justify-center">
    ...
    <div class="mb-6">
    <form action="{{  route('search_class_students')  }}" method="post" role="search">
      @csrf
      ...
      <div class="input-group">
        <label class="text-xl">Search Students: </label>
          <input type="text" class="form-control pt-4 pb-4 pr-12 pl-2 ml-2 text-xl" name="q" placeholder="Search students">
              <button type="submit" class="bg-purple-400 text-white px-8 py-4 border rounded-md hover:bg-white hover:border-indigo-500 hover:text-black">Search Students
              </button>
      </div>
    </form>
    </div>
